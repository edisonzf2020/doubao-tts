...
| GET | `/ui` | 音色预览页面（免鉴权，见下） |
| GET | `/ui/voices` | UI 用的音色列表（按女声/男声分组，免鉴权） |

### 音色预览页面

浏览器打开 `http://<host>:<port>/ui`，按女声/男声分类浏览全部音色，
每个音色右侧有播放按钮，点击试听一句约 20 字的示例语音。

试听走 `/v1/audio/speech`，**必须在页面顶部填入 API Key**——没有 key 无法
试听，避免被滥用。Key 只存在浏览器内存里（刷新即清空），不落盘。
页面本身和音色列表免鉴权（只是元数据，不消耗账号资源）。
